...
class ChunkedTokenDatabase(TokenDatabase):
    ...
    def process_tokens_from_prefix(
        ...
        *,
        ...
        """Process only full-request tokens after a cached chunk prefix.

        Args:
            tokens: Complete request token sequence.
            prefix_token_count: Chunk-aligned number of tokens already covered
                by ``prefix_hash``.
            prefix_hash: Chained hash after ``prefix_token_count`` tokens.
            make_key: Return cache keys when true, otherwise return hashes.
            request_configs: Optional request-specific cache-key configuration.
            kv_group: KV group used to construct cache keys.

        Yields:
            Absolute ``(start, end, key_or_hash)`` tuples for suffix chunks.

        Raises:
            ValueError: If the cached prefix is negative, unaligned, or longer
                than the supplied token sequence.
        """
