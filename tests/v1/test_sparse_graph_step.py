    ...
    def __init__(
        ...
        *,
        ...
        self.lmcache_engine = SimpleNamespace(
            num_layers_for_group=lambda group: self.num_layers,
...
@pytest.mark.parametrize("index_layers", [1, 2])
def test_shared_indexer_model_uses_registered_group1_cardinality(index_layers: int) -> None:
    adapter = FakeAdapter(native_index=True)
    adapter.lmcache_engine.num_layers_for_group = lambda group: (
        index_layers if group == 1 else adapter.num_layers
    )
    adapter._kvcaches_for_group = lambda group: [object()] * (
        index_layers if group == 1 else adapter.num_layers
    )
    result = adapter.prepare_sparse_graph_step(
        ("layers.0.attn", "layers.1.attn"), request_ids=("r1",), frontiers=(512,)
    )
    assert result == (adapter.source,)
    assert not adapter.waits and not adapter.state.indexer_npu_resident


def test_shared_indexer_model_cannot_hide_missing_registered_layers() -> None:
    adapter = FakeAdapter(native_index=True)
    adapter.lmcache_engine.num_layers_for_group = lambda group: 2 if group == 1 else adapter.num_layers
    adapter._kvcaches_for_group = lambda group: [object()] * (1 if group == 1 else adapter.num_layers)
    with pytest.raises(RuntimeError, match="preparation failed"):
        adapter.prepare_sparse_graph_step(("layers.0.attn", "layers.1.attn"))
