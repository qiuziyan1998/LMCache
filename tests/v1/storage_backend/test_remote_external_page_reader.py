...
def test_terminal_read_error_releases_owners_without_cyclic_gc(
    ...
    async def fail(*args: Any) -> None:
        raise error_type("known terminal failure")
    ...
    backend.config = SimpleNamespace(prefill_group0_direct_hbm=True)
...
def test_short_read_connector_releases_owners_without_cyclic_gc() -> None:
    ...
    async def run() -> None:
        ...
        connector.config = SimpleNamespace(
            transfer_timeout=1, prefill_group0_direct_hbm=True
        )
...
@pytest.mark.parametrize(
    "outcome",
    ["ok", "failure", "completed_ok", "completed_failure", "pending", "unknown"],
)
def test_external_read_keeps_success_flow_and_unknown_owner_latch(
    monkeypatch: pytest.MonkeyPatch, enabled: bool, outcome: str
) -> None:
    """The P cleanup policy is consulted only on known failed reads."""
    ...
    class Config:
        @property
        def prefill_group0_direct_hbm(self) -> bool:
            policy_reads.append(True)
            return enabled

    future = Mock()
    future.done.return_value = outcome != "pending"
    unknown = NativeExternalPageTransferUnknownError("get", future)
    failure = RuntimeError("terminal read failure")
    future.result.side_effect = {
        "ok": [None],
        "failure": [failure],
        "completed_ok": [TimeoutError(), None],
        "completed_failure": [TimeoutError(), failure],
        "pending": [TimeoutError()],
        "unknown": [unknown],
    }[outcome]
    submitted = object()
    submit = Mock(return_value=future)
    monkeypatch.setattr(remote_backend.asyncio, "run_coroutine_threadsafe", submit)
    backend = object.__new__(RemoteBackend)
    backend.config = Config()
    backend.loop = object()
    backend.connection = SimpleNamespace(
        batched_get_external_pages=Mock(return_value=submitted)
    )
    backend._mla_worker_id_as0_mode = False
    backend._external_page_outer_timeout_secs = lambda: 2
    owners, ptrs, sizes = (object(),), [[1]], [[8]]
    args = (["key"], ptrs, sizes, owners, "request")
    if outcome in ("ok", "completed_ok"):
        backend.batched_get_external_pages(*args)
        assert policy_reads == []
    elif outcome in ("pending", "unknown"):
        with pytest.raises(NativeExternalPageTransferUnknownError) as caught:
            backend.batched_get_external_pages(*args)
        assert caught.value.terminal_future is future
        assert backend._external_page_fatal_error is caught.value
        with pytest.raises(NativeExternalPageTransferUnknownError) as repeated:
            backend.batched_get_external_pages(*args)
    ...
    assert backend._strict_external_destination_ownership
    backend.connection.batched_get_external_pages.assert_called_once_with(*args)
    submit.assert_called_once_with(submitted, backend.loop)
    assert future.result.call_count == (2 if outcome.startswith("completed") else 1)
