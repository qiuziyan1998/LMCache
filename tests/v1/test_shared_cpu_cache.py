...
def test_page_first_batch_plan_handles_partial_tail(
    ...
    assert resolved["planned_page_chunks"] == (2 if merged_pages else 3)
...
def test_runtime_capacity_skips_hot_cache_scan_for_zero_allocation():
    class _FailOnItems(dict):
        def items(self):
            raise AssertionError("zero-allocation preflight scanned hot cache")

    engine = _make_engine_for_sparse_capacity(max_local_cpu_size=1)
    hot_key = _make_key()
    hot_obj = _make_memory_obj(
        torch.empty(1024, dtype=torch.uint8), physical_size=64
    )
    backend = _FakeLocalCPUBackend(
        free_bytes=0, hot_cache=_FailOnItems({hot_key: hot_obj})
    )
    engine._shared_local_cpu_backend = lambda: backend
