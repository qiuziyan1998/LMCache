...
def make_request(
    ...
    *,
    ...
    impl._supports_dsa_split_layout = lambda: True
...
@pytest.mark.parametrize("prompt", [819199, 819200])
...
@pytest.mark.parametrize("prompt", [256, 4096, 10000, 819201])
def test_short_prompt_does_not_get_cold_hit_adjustment(
    callbacks: SimpleNamespace, prompt: int
) -> None:
    """Preserve the target's short-prompt and block-count-mismatch dense path."""
